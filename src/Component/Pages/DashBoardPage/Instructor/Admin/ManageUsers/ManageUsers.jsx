          ...
          <tbody>
            {/* row 1 */}
            {
                users.map((user,index) => <tr key={user._id}>
                    ...
                    <td>{user.role}</td>
                    <td>
                      <button disabled={disableAdmin.includes(index)} onClick={() => handleAdmin(user,index)} className="btn text-xs bg-green-300 btn-sm">Admin</button>
                    </td>
                    <td>
                      <button disabled={disableInstructor.includes(index)} onClick={() => handleInstructor(user,index)} className="btn text-xs bg-red-300 btn-sm">instructor</button>
                    </td>
                  </tr>)
            }
          </tbody>
