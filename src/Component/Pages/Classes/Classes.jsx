...
const Classes = () => {
  ...
  const handleSelectedClass = (item) => {
    console.log(item)
    const selectedClass = {className: item.className,classImg: item.classImg,email: item.email,name: item.name,price: item.price,seats: item.seats,classId: item._id }
  ...
  return (
    <div className=" md:w-[1280px] mx-auto w-full bg-blue-100 rounded-xl ">
      ...
      <div className="md:p-10 p-3 grid md:grid-cols-3 grid-cols-1 gap-10">
        {/* card  */}
        {
          classes.map(data => <div key={data._id} className="h-[500px] rounded-xl shadow-xl w-full md:w-[350px] bg-white">
          ...
          <div className="h-[230px] flex flex-col justify-between px-4 w-full">
            ...
            <div>
              ...
              <button onClick={() => handleSelectedClass(data)} className="flex items-start mt-3 bg-red-400 text-white hover:text-black rounded-full w-1/2 px-[32px] text-sm font-bold py-1 hover:bg-red-300 duration-500 active:bg-blue-200 active:duration-150">
                {" "}
                Enroll Now
              </button>
            </div>
